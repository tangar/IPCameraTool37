...
class CameraConfig:
    def __init__(self, config_file="config.json"):
        ...
        self.default_config = {
            ...
            "main_rtsp_url": "rtsp://admin:6m8vw@192.168.37.3:554/cam/realmonitor?channel=1&subtype=0&unicast=true&proto=Onvif",
            "second_rtsp_url": "rtsp://admin:6m8vw@192.168.37.4:554/stream0",
    ...
    def load_config(self):
        ...
        self.controller_ip = config.get("controller_ip", config["controller_ip"])
        self.controller_port_tx = config.get("controller_port_tx", config["controller_port_tx"])
        self.controller_port_rx = config.get("controller_port_rx", config["controller_port_rx"])
        self.main_rtsp_url = config.get("main_rtsp_url", config["main_rtsp_url"])
        self.second_rtsp_url = config.get("second_rtsp_url", config["second_rtsp_url"])
        self.CAMERA_SIDE_HOST = config.get("CAMERA_HOST", config["CAMERA_SIDE_HOST"])
        self.CAMERA_SIDE_ONVIFPORT = config.get("CAMERA_PORT", config["CAMERA_SIDE_PORT"])
        self.CAMERA_SIDE_USER = config.get("CAMERA_USER", config["CAMERA_SIDE_USER"])
        self.CAMERA_SIDE_PASS = config.get("CAMERA_PASS", config["CAMERA_SIDE_PASS"])
        self.CAMERA_DN_HOST = config.get("CAMERA_HOST", config["CAMERA_DN_HOST"])
        self.CAMERA_DN_ONVIFPORT = config.get("CAMERA_PORT", config["CAMERA_DN_PORT"])
        self.CAMERA_DN_USER = config.get("CAMERA_USER", config["CAMERA_DN_USER"])
        self.CAMERA_DN_PASS = config.get("CAMERA_PASS", config["CAMERA_DN_PASS"])
        self.SAVE_PATH = config.get("SAVE_PATH", config["SAVE_PATH"])
